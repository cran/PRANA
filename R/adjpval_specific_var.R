#' @title adjpval_specific_var
#'
#' @description A function to retrieve a vector of adjusted p-values after running PRANA.
#'
#' @param  adjptab A table that includes adjusted p-values for a specific variable.
#' @param varname Specify the name of the variable of interest.
#'
#' @return A vector of adjusted p-values for a single variable from the model.
#'
#' @examples
...
adjpval_specific_var <- function(adjptab, varname) {
        adjpvalvec <- adjptab[varname]
        return(adjpvalvec = adjpvalvec)
}
